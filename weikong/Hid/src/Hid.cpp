...
int CHid::open(unsigned short vid, unsigned short pid, const char* szPath, const wchar_t *serial_number)
{
	...
	while (cur_dev) {
		if (cur_dev->vendor_id == vid && cur_dev->product_id == pid) {
			if (serial_number) {
				...
				if (strcmp(szPath, cur_dev->path) == 0) {
					path_to_open = cur_dev->path;
					break;
				}
			} else {
...
int CHid::open_path(const char *path)
{
	...
	res = m_dllFuncs.getCaps(pp_data, &caps);
#if 0
	if (res == 0) {
		retCode = -5;
		goto err_pp_data;
...
int CHid::get_indexed_string(int str_index, wchar_t *str, size_t maxlen)
{
	...
	res = m_dllFuncs.getIndexedString(m_device.device_handle, str_index, str, sizeof(wchar_t) * MIN(maxlen, MAX_STRING_WCHARS));
	if (!res) {
		return -3;
	}
...
int CHid::get_manufacturer_string(wchar_t *str, size_t maxlen)
{
	...
	res = m_dllFuncs.getManufactureString(m_device.device_handle, str, sizeof(wchar_t) * MIN(maxlen, MAX_STRING_WCHARS));
	if (!res) {
		return -3;
	}
...
int CHid::get_product_string(wchar_t *str, size_t maxlen)
{
	...
	res = m_dllFuncs.getProductString(m_device.device_handle, str, sizeof(wchar_t) * MIN(maxlen, MAX_STRING_WCHARS));
	if (!res) {
		return -3;
	}
...
int CHid::get_serial_number_string(wchar_t *str, size_t maxlen)
{
	...
	res = m_dllFuncs.getSerialNumberString(m_device.device_handle, str, sizeof(wchar_t) * MIN(maxlen, MAX_STRING_WCHARS));
	if (!res) {
		return -3;
	}
...
int CHid::write(const void *data, size_t length, size_t millsseconds)
{
	BOOL res;
	DWORD bytes_write = 0;
	hid_device *dev = &m_device;
	...
	res = WaitForSingleObject(ev, millsseconds);
	if (res == WAIT_TIMEOUT) {
		return -3;
	}
...
int CHid::read(void *data, size_t length, size_t milliseconds)
{
    DWORD bytes_read = 0;
    size_t copy_len = 0;
	char *p_copy_head = NULL;
    BOOL res;
	hid_device *dev = &m_device;
	...
	res = GetOverlappedResult(dev->device_handle, &dev->ol_read, &bytes_read, TRUE);
	if (res && bytes_read > 0) {
		if (dev->read_buf[0] == 0x0) {
			...
			bytes_read--;
			p_copy_head = dev->read_buf + 1;
		}else{
			p_copy_head = dev->read_buf;
		}

		copy_len = min(length, bytes_read);
		memcpy(data, p_copy_head, copy_len);
		return copy_len;
	} else {
		return -4;
	}
