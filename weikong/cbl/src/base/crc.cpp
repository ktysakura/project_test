...
static const unsigned long long int crc64Table[256] = {
...
unsigned long long int crc64(const void *inBuf, size_t bytes) 
{
	...
	unsigned char *s = (unsigned char *)inBuf;

	if (s) {
		for (size_t j = 0; j < bytes; j++) {
			unsigned char byte = s[j];
			crc = crc64Table[(unsigned char)crc ^ byte] ^ (crc >> 8);
		}
	}
...
{
	int i;
	unsigned char *s = (unsigned char *)pbuf;
